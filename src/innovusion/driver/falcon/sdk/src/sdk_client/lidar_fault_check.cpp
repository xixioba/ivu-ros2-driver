 ...
 * @param  has_force_xyz  has been converted to xyz point
...
InnoFrameCheckProcess InnoGalvoMirrorCheck::galvo_mirror_offset_check(
  ...
  if (last_frame_idx_ != pkt.idx) {
    ...
    if (frame_start_speed_ >= kInnoMinGalvoVehicleSpeed
  ...
  if (need_ground_points_) {
    collect_ground_points_(pkt, has_force_xyz);
  }
...
int InnoGalvoMirrorCheck::collect_ground_points_(
  ...
  int ret = 0;
  if (has_force_xyz) {
    ret = collect_ground_points_process_(pkt);
  } else {
    if (InnoDataPacketUtils::convert_to_xyz_pointcloud(
                    ...
                    false)) {
      ret = collect_ground_points_process_(xyz_data_packet_);
    } else {
...
int InnoGalvoMirrorCheck::cal_and_get_galvo_check_result_(
  ...
  if (curr_speed < kInnoMinGalvoVehicleSpeed) {
    check_result->frame_check_code
                        = INNO_GALVO_CHECK_FRAME_CODE_INVALID_SPEED;
    check_result->fault_status = INNO_GALVO_FAULT_STATUS_INVALID_CHECK_SKIP;
    if (curr_speed > kInnoMaxGalvoCheckResetSpeed) {
      check_angles_[check_angle_idx] = 0;
      ++check_times_;
    } else {
      /* the speed of vehicle too low to reset check times */
      reset_check_times();
    }
    return -1;
  }
...
int InnoMaxDistanceCheck::check_max_distance(
  ...
  if (time_diff > kInnoUpdateSpeedMaxDelayTime) {
    ...
    restart_check();
    return -1;
  } else if (vehicle_speed_ < threshold_start_check_speed) {
    // Speed to0 low to skip and reset
    ...
    if (check_idx_ < max_check_times) {
      ...
      if (vehicle_speed_ >= threshold_min_speed
    ...
    if (check_idx_ >= max_check_times) {
      ...
      if (valid_check_count_ < threshold_check_times) {
      ...
      } else if (check_result->mean_refl >= threshold_normal_refl) {
