...
int LidarCommunication::StatusCounters::set(const char *input) {
  ...
  int ret = sscanf(input,
                   "time_config=%d up_time=%" PRI_SIZEX
                   " err=0x%x stream_status=%d "
                   "stream_count=%u "
                   "data_sent=%" PRI_SIZELU " count1=%u count2=%u count3=%u",
...
LidarCommunication::LidarCommunication(const char *ip_addr,
                                       uint16_t port,
...
int LidarCommunication::send_command_and_save_reply_(char *buffer,
                                                     size_t buffer_len,
                                                     const char *cmd) {
...
double LidarCommunication::get_config_section_key_value(const char *section,
                                                        const char *key,
                                                        double default_value) {
  double ret = default_value;
  char *config_buffer = send_command_and_get_reply("%s %s %s",
                                                   "get_i_config",
                                                   section, key);
  if (config_buffer != NULL) {
    ...
    if (sscanf(config_buffer, pat, &value) == 1) {
      ...
      inno_log_error("invalid return for %s %s, %s, use default value %f",
                     section, key, config_buffer, ret);
    }
    free(config_buffer);
  } else {
    inno_log_error("cannot get %s %s, use default value %f",
                   section, key, ret);
  }
  return ret;
}

int LidarCommunication::set_config_section_key_value(const char *section,
                                                     const char *key,
                                                     const char *value) {
  char *config_buffer = send_command_and_get_reply("%s %s %s %s",
                                                   "set_i_config",
                                                   section, key, value);
  if (config_buffer != NULL) {
    free(config_buffer);
    ...
    inno_log_error("cannot set %s %s to %s",
                   section, key, value);
    return -1;
  }
}
...
int LidarCommunication::reg_read(uint32_t addr, uint32_t *data) {
  ...
  if (recv_buffer == NULL) {
    ...
    if (sscanf(recv_buffer, "reg_rd: %x %x", &addr, data) != 2) {
      inno_log_error("Failed to reg_read 0x%x, invalid reply %s",
                     addr, recv_buffer);
    } else {
      ret = 0;
    }
...
int LidarCommunication::reg_write(uint32_t addr, uint32_t data) {
  if (send_command_and_free_reply("reg_wr %x %x",
                                  addr, data) < 0) {
...
int LidarCommunication::get_did(char *buffer, size_t buffer_len,
                                const char* did) {
  ...
  if (recv_buffer == NULL) {
    ...
    if (buffer_len < len - (ignore_chars - 1)) {
      ...
      #else
      inno_log_warning("DID buffer too small %zu < %zu", buffer_len,
                       len - (ignore_chars - 1));
      #endif
      free(recv_buffer);
      return -3;
...
int LidarCommunication::get_model(char *buffer, size_t buffer_len) {
  inno_log_verify(buffer_len >= 2,
                  "buffer_len too small %" PRI_SIZELU "", buffer_len);
  ...
  if (recv_buffer == NULL) {
    ...
    if (sscanf(recv_buffer, "model: %c", &model) != 1) {
      ...
      // successful
      inno_log_verify(buffer_len >= 2, "buffer_len %" PRI_SIZELU,
                      buffer_len);
      free(recv_buffer);
      buffer[0] = model;
      buffer[1] = 0;
...
int LidarCommunication::get_geo_yaml(char *buffer, size_t buffer_len) {
  int expect_md5 = 1;
  int ret = recv_length_buffer(buffer, buffer_len,
                               expect_md5,
                               "download_cal_file 0");
  if (ret < 0) {
...
int LidarCommunication::get_ext_packet_format(bool *is_extended_format) {
  inno_log_verify(is_extended_format,
                  "is_extended_format should not be NULL");
  uint32_t data;
...
int LidarCommunication::get_status(StatusCounters *counters,
                                   bool get_diff) {
  inno_log_verify(counters,
                  "counters should not be NULL");
  ...
  if (counters->set(recv_buffer)) {
    inno_log_error("Cannot get_status, invalid buffer %s",
                   recv_buffer);
    free(recv_buffer);
    return -1;
  } else {
...
int LidarCommunication::set_vertical_roi(double vertical_roi_angle) {
  return send_command_and_free_reply("set_vertical_roi %f",
                                     vertical_roi_angle);
}
...
int LidarCommunication::get_vertical_roi(double *vertical_roi_angle) {
  ...
  if (recv_buffer == NULL) {
    ...
    if (sscanf(recv_buffer, "%lf", vertical_roi_angle) != 1) {
      inno_log_error("Failed to get_vertical_roi, invalid reply %s",
                     recv_buffer);
      ret = -2;
    } else {
      ret = 0;
...
int LidarCommunication::set_mode(enum InnoLidarMode mode,
  ...
  char *recv_buffer = send_command_and_get_reply("set_mode %d",
                                                 mode);
  if (recv_buffer == NULL) {
...
int LidarCommunication::get_calibration_mode_id(int *calibration_id) {
  ...
  if (sscanf(recv_buffer, "%d", calibration_id) != 1) {
    inno_log_error("bad reply for get_calibration_mode_id %s",
                    recv_buffer);
    free(recv_buffer);
    return -2;
  }
  if (*calibration_id != 0 && *calibration_id != 1) {
    inno_log_error("invalid calibration mode id %s",
                    recv_buffer);
    free(recv_buffer);
    return -2;
  }
...
int LidarCommunication::open_streaming(void) {
  {
    std::unique_lock<std::mutex> lk(mutex_);
    inno_log_verify(streaming_fd_ == -1,
                    "streaming_fd is already %d",
                    streaming_fd_);
  }
...
int LidarCommunication::start_streaming(bool is_tcp) {
  inno_log_verify(streaming_fd_ >= 0,
                  "streaming_fd %d is not valid",
                  streaming_fd_);
  int zero = 0;
  int ret = send_command_with_fd(streaming_fd_, NULL, &zero,
                                 is_tcp ? "start %hu" : "start direct %hu",
                                 service_port_);
  if (ret < 0) {
