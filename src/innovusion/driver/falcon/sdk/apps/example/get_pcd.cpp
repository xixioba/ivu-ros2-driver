...
class FileRecorder {
 ...
 public:
  ...
  static const size_t kMaxCsvSize_ = 130;
...
class ExampleProcessor {
 public:
  ...
  static void message_callback_s(const int lidar_handle, void *ctx,
    ...
    if (level == INNO_MESSAGE_LEVEL_DEBUG
       && code == INNO_MESSAGE_CODE_GALVO_MIRROR_CHECK_RESULT) {
  ...
  */
  void convert_and_process_data_xyz_point_(const InnoDataPacket &pkt) {
    InnoDataPacket *new_pkt =
        InnoDataPacketUtils::convert_to_xyz_pointcloud_malloced(pkt);
    inno_log_verify(new_pkt, "new_pkt");
...
int main(int argc, char **argv) {
  ...
  inno_log_info("####simulation_galvo_check=%d", simulation_galvo_check);
